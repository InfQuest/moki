...
const CJK_LANGUAGES = ["zh-CN", "zh-TW", "ja", "ko"];
const CHINESE_LANGUAGES = ["zh-CN", "zh-TW"];
function isCJKLanguage(langCode) {
  return CJK_LANGUAGES.includes(langCode);
}
function isChineseLanguage(langCode) {
  return CHINESE_LANGUAGES.includes(langCode);
}
const POS_LABELS = {
  // Core POS
  noun: {
...
function getPosLabel(pos, targetLang) {
  if (!pos) return "";
  const labels = POS_LABELS[pos];
  if (!labels) {
    return pos;
  }
  const label = labels[targetLang];
  if (label) {
    return label;
  }
...
//# sourceMappingURL=chunk-Idcw88YU.js.map
